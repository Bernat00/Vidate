  ...
  return (
    <div className="bg-gradient-to-t from-bgAccentPrimary to-bgAccentSecondary flex items-center justify-center min-h-screen">
      <div className="w-full max-w-md bg-bgPrimary border border-borderAccent rounded-2xl shadow-2xl p-8 m-4">
        ...
        <h2 className="text-textPrimary text-xl font-semibold text-center mb-6">Sign in to your account</h2>
        ...
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
          <div>
            ...
            <input id="email" placeholder="name@company.com" className={`w-full p-2.5 rounded-lg bg-bgSecondary border text-textPrimary focus:outline-none focus:ring-2 ${errors.email ? 'border-textError focus:ring-textError' : 'border-borderAccentLight focus:ring-borderAccent'}`} {...register('email', { required: 'Email is required', pattern: { value: /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i, message: 'Invalid email address' } })} />
          ...
          <div>
            ...
            <input type="password" id="password" placeholder="••••••••" className={`w-full p-2.5 rounded-lg bg-bgSecondary border text-textPrimary focus:outline-none focus:ring-2 ${errors.password ? 'border-textError focus:ring-textError' : 'border-borderAccentLight focus:ring-borderAccent'}`} {...register('password', { required: 'Password is required' })} />
          ...
          <div className="flex items-center justify-between text-textSecondary">
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" className="w-4 h-4 rounded border-borderAccentLight bg-bgSecondary focus:ring-borderAccent" {...register('rememberMe')} />
              Remember me
            </label>
            <Link to="/forgot-password" className="text-textAccent hover:underline text-sm">Forgot password?</Link>
          </div>

          <button type="submit" className="w-full bg-bgAccentSecondary hover:bg-borderAccent text-textPrimary font-semibold rounded-lg py-2.5 transition hover:cursor-pointer">Sign in</button>

          <p className="text-textSecondary text-sm text-center">Don’t have an account? <Link to="/register" className="text-textAccent hover:underline">Sign up</Link></p>
        </form>
      </div>
    </div>
  );
